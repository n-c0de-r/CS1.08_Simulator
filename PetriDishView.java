...
/**
 * A GUI for the petri dish, with runtime controls.
 * 
 * @author David J. Barnes and Michael Kölling
 * @version  2016.02.29
 */
public class PetriDishView extends JFrame
{
    // The longest delay for the animation, in milliseconds.
    private static final int LONGEST_DELAY = 1000;
    // Colors for the different virus states.
    ...
    private final PetriDish petri;
    private boolean running;
    private int delay;
    
    /**
     * Constructor for objects of class PetriDishView
     * @param petri
     */
    public PetriDishView(PetriDish petri, int rows, int cols)
    {
        super("2D Virusular Automaton");
    ...
    /**
     * Show the states of the viruses.
     */
    public void showViruses()
    {
        Virus[][] viruses = petri.getViruses();
        if(!isVisible()) {
            setVisible(true);
        }
        
        view.preparePaint();
        for(int row = 0; row < viruses.length; row++) {
            Virus[] virusRow = viruses[row];
            int numCols = virusRow.length;
            for(int col = 0; col < numCols; col++) {
                int state = virusRow[col].getState();
